...
import { buildSubject, generateFollowUpEmailBody, buildFollowUpSubject, type OriginalEmail } from "@/lib/email/render";
...
export async function POST(
  ...
  try {
    ...
    if (rfq.status !== "sent" && rfq.status !== "open") {
      return NextResponse.json({ error: "RFQ must be sent or open to send follow-ups" }, { status: 400 });
    }
    ...
    const followUpSubject = buildFollowUpSubject(originalSubject);
    ...
    for (const link of pendingSupplierLinks) {
      ...
      const bodyText = generateFollowUpEmailBody(formData, company, link.supplierName, portalLink, originalEmail);
      const mailtoUrl = generateMailtoUrl(link.supplierEmail, "", followUpSubject, bodyText);
      const messageId = `<followup-${Date.now()}-${Math.random().toString(36).slice(2)}@${company.email?.split("@")[1] ?? "bid.elite-n.com"}>`;
